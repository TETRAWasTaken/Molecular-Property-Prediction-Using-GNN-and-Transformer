...
class MolecularPropertyPipeline:
    ...
    def canonicalize_smiles(
        ...
        num_workers = self._resolve_worker_count(n_jobs)
    ...
    def generate_graphs(
        ...
        if verbose:
            print(f"Using {num_workers} threads for graph generation on {num_molecules} molecules")

        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            if show_progress:
                results = []
                for i, result in enumerate(executor.map(MolecularPropertyPipeline._smiles_to_graph, args_list)):
                    results.append(result)
                    self._print_progress(i + 1, num_molecules)
            else:
                results = list(executor.map(MolecularPropertyPipeline._smiles_to_graph, args_list))
