...
class GraphEncoder(torch.nn.Module):
    """
    Graph feature extractor based on the Graph Isomorphism Network (GIN).
    Strips away the prediction head to output a continuous structural embedding.
    """
    def __init__(self, node_in_dim: int = 6, edge_in_dim: int = 3,
                 hidden_dim: int = 128, num_layer: int = 5, dropout: float = 0.2):
        ...
        # Learnable virtual node for global graph communication
    ...
    def forward(self, data: torch_geometric.data.Data) -> torch.Tensor:
        """
        Passes graph data through GIN layers.
        Returns:
            torch.Tensor: The global graph embedding of shape [batch_size, hidden_dim]
        """
        ...
        # Extract the final pooled representation (128-dim vector)
